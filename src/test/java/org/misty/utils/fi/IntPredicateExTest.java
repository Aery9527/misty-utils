...
class IntPredicateExTest {
    ...
    void execute() {
        ...
        IntPredicateEx predicateEx = (arg) -> arg % 2 == 0;
        Assertions.assertThat(predicateEx.execute(forTestReturn)).isEqualTo(forTestReturn % 2 == 0);

        IntPredicateEx predicateEx1 = (arg) -> FunctionalInterfaceExTest.throwIOException(); // there is no need for a try-catch.
        testThrow(predicateEx1, IOException.class);

        IntPredicateEx predicateEx2 = (arg) -> FunctionalInterfaceExTest.throwInterruptedException(); // there is no need for a try-catch.
